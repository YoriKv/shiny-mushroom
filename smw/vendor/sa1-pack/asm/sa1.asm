...
; LOCAL MODIFICATION (shiny-mushroom): defaults for the switches below, set
; only when the caller has not already defined them on the command line.
if defined("remap_dp") == 0
	!remap_dp = 1
endif
...
; LOCAL MODIFICATION (shiny-mushroom, intended for upstream).
;
; The work-RAM remaps are switchable. A base whose *source* already places
; work RAM at the SA-1 addresses has no need of them, and without a switch
; there is no safe way to say so: these blocks are gated by guards that read
; bytes only a clean ROM has, and a guard that does not match skips its block
; and reports nothing. A source-level remap moves some of those bytes, so the
; guard silently turns the block off half-applied. An explicit switch replaces
; a silent failure with a stated intent.
;
; Both default to 1, so an unmodified build is byte-for-byte what it was.
if !remap_dp
...
; LOCAL MODIFICATION (shiny-mushroom, intended for upstream): probe this
; block's own output rather than a byte remap/addr.asm happens to rewrite.
; The intent is idempotency -- do not write these twice -- and $00:8A60 says
; that directly. Borrowing addr.asm's probe means any base that remaps work
