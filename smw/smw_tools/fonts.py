...
A string in the disassembly is a ``db "..."`` under an asar ``table``
directive, and the table is what turns each character into the tile the
game draws: ``tables/fonts/standard.txt`` says ``A=00``, so ``"A"`` assembles
to ``$00``. Nothing in the assembled cartridge remembers the character, only
...
**A table is one direction at a time.** ``standard.txt`` maps ``\\`` to
...
class FontTable:
    """One asar table file: ``<char>=<hex>`` per line, one tile a character."""
    ...
    def parse(cls, text: str) -> FontTable:
        """A table file's text. Blank lines are nothing; anything else is
        ``char=hex``, with the hex as many digits as the tile is wide."""
        tiles: dict[str, int] = {}
        for number, raw in enumerate(text.split("\n"), start=1):
            line = raw.rstrip("\r")
            if not line.strip():
                continue
            # The one character that is the separator too: `==36` maps `=`.
            char, sep, value = (
                ("=", "=", line[2:]) if line.startswith("==") else line.partition("=")
            )
            if not sep or len(char) != 1:
                raise FontError(f"line {number}: {line!r} is not char=hex")
            try:
                tiles[char] = int(value.strip(), 16)
            except ValueError:
                raise FontError(f"line {number}: {value!r} is not hex") from None
        return cls(tiles)
...
#: The table every international string assembles through, relative to the
#: base's game folder.
STANDARD = Path("tables/fonts/standard.txt")
