...
; Internal ROM header. Contains a brief summary of the game. See details.
.ProgramTitle:		db "!Define_Global_InternalName"	; Internal ROM name
padbyte $20
...
.ProgramLayout:		db !ROMLayoutHeaderByte	; LoROM, slow
.ProgramContents:	db !Define_Global_ROMType	; ROM + SRAM + Battery
.ProgramSize:		db !ROMSize	; <= 4Mb ROM
.ProgramMemorySize:	db !Define_Global_SRAMSize	; 16Kb SRAM
.ProgramRegion:		db !Define_Global_Region	; Country code: NTSC
if !Define_Global_CartridgeHeaderVersion == $02
			db $33
else
.ProgramLicensee:	db !Define_Global_LicenseeID	; Nintendo EAD
endif
.ProgramVersion:	db !Define_Global_VersionNumber	; Version: zero
.ProgramComplement:	dw !Define_Global_ChecksumCompliment	;\ Note that the SS checksum and complement are wrong
.ProgramChecksum:	dw !Define_Global_Checksum	;/ They should be $B4CF and $4B30
.Unused1:		dw !UnusedNativeModeVector1	;\ Fairly certain these differences have to do with
.Unused2:		dw !UnusedNativeModeVector2	;| the weird freespace pattern & stray bits
.NativeModeCOP:		dw !NativeModeCOPVector	;|
.NativeModeBRK:		dw !NativeModeBRKVector	;| Except for this $50B2 maybe?
...
.EmulationModeBRK:	dw !EmulationModeBRKVector	; Never used, would be emulation mode BRK vector
...
.EmulationModeIRQ:	dw !EmulationModeIRQVector	; Emulation mode IRQ and BRK vector
...
; Called with the parentheses asar requires -- upstream writes %GetROMSize
; bare, which only ever assembled because ApplyAsarPatches ships off.
%GetROMSize()
